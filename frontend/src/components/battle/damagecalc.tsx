...
export const DamageCalculation = async (
  ...
  try {
    const attackStat =
      attackingPokemon.stats.find((stat) => stat.stat.name === "attack")
        ?.base_stat || 50;
    const defenseStat =
    ...
    const move1Stats = await axios.get(
      `https://pokeapi.co/api/v2/move/${move1Name}`,
    );
    const move2Stats = await axios.get(
      `https://pokeapi.co/api/v2/move/${move2Name}`,
    );

    const damage = {
      move1: Math.floor(
        ((attackStat / defenseStat) * move1Stats.data.power) / 10 + 4,
      ),
      move2: Math.floor(
        ((attackStat / defenseStat) * move2Stats.data.power) / 10 + 4,
      ),
    };
